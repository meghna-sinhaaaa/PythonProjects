...
def get_contact_page(soup, website):
    # contact_links = soup.find_all('a', href=re.compile(r'(?i)contact'))
                ...
                for ul_tag in soup.find_all("ul"):
                    for div_tag in ul_tag.find_all("div"):
                        for li_tag in div_tag.find_all("li"):
                            for link in li_tag.find_all(
                                    lambda tag: tag.name == "a" and "contact" in tag.get("href", "")):
                                contact_links.append(link)
                if len(contact_links) == 0:
...
def mainmod(website):
    ...
    if resp == "Error":
        ...
        if answer is None:
            print("Checking sumit for : {}".format(website))
